...
public class Akcija implements Serializable {
	...
	private static final long serialVersionUID = -7702247233114394267L;
		...
		for (Stanje stanje : stanjaDka) {
			for (Map.Entry<String, Integer> prijelaz : stanje.getPrijelazi().entrySet()) {
				...
				if (zavrsniIKraj.contains(znak))
					tablica[stanje.getIndex()][zavrsniIKraj.indexOf(znak)] = new Par(AkcijaParsera.POMAKNI, novoStanje);
			}
			...
			for (LR1Stavka stavka : stanje.getSadrzaj()) {
				if (stavka.getPrijelaz() == null) {// potpuna LR stavka
					...
					if (stavka.getZnakLijeveStraneProdukcije().equals("<%>")) {
						tablica[stanje.getIndex()][zavrsniIKraj.indexOf("#")] = new Par(AkcijaParsera.PRIHVATI, 0);
					}
						...
						// i reduciraj/reduciraj: za prvi slucaj izaberi
						// pomakni,
						...
						for (String znakIzaProdukcije : stavka.getZnakoviIzaProdukcije()) {
							...
							if (promatraniPar == null) {
								tablica[stanje.getIndex()][indeksZnakaIzaProdukcije] = new Par(AkcijaParsera.REDUCIRAJ,
										stavka.getIndeksProdukcije());
